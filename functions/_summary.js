...
// v5: em dash 기계적 치환(프롬프트 금지 지시만으로는 36개 기업 중 2개에서 계속 나왔다).
...
- 온점(.)을 쓰지 않는다. 끝에도 중간에도 쓰지 않으며, 쉼표로 이어 한 덩어리로 만든다. 단 제품명·버전 표기(A.X K1, 알리 2.0.0, Apache 2.0) 안의 온점은 원문대로 둔다
...
// 저장 전 문체 정규화. 지시만으로는 모델이 계속 넣는 항목(끝 온점·em dash)을 기계적으로 처리한다.
const clean = (s) => stripTrailingPeriod(replaceEmDash(s));
...
export async function generateAndStore(env, name) {
  ...
  for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
    try {
      ...
      // 끝 온점·em dash 는 문체 기준 위반이라 저장 전에 정규화한다(모델이 습관적으로 넣는 경우가 있음).
      // period 라벨은 "2026.1" 같은 표기가 있을 수 있어 건드리지 않는다.
      const flow = Array.isArray(obj.flow) ? obj.flow.filter((x) => x && x.text).slice(0, 6).map((x) => ({ period: String(x.period || '').slice(0, 20), text: clean(String(x.text).slice(0, 300)) })) : [];
      const insight = Array.isArray(obj.insight) ? obj.insight.filter(Boolean).slice(0, 4).map((x) => clean(String(x).slice(0, 400))) : [];
