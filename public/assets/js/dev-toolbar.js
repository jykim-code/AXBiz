/* dev 프리뷰 배너 — ?preview=1 일 때만 활성(아니면 no-op).
   ...
   index/company/explore/(radar) 에 <script> 로 포함. 검수·편집은 /dev 콘솔에서. */
(function () {
  ...
  if (!getPin()) {
    bar.innerHTML = '<span style="font-weight:700;letter-spacing:.08em;font-size:10px;text-transform:uppercase;background:#c8f200;color:#111;border-radius:999px;padding:2px 8px">DEV 프리뷰</span>' +
      ...
      '<a href="/dev" style="color:#c8f200;font-weight:600;text-decoration:none">검수 콘솔에서 PIN 입력 →</a>';
    return;
  }
  ...
  bar.innerHTML = '<span style="font-weight:700;letter-spacing:.08em;font-size:10px;text-transform:uppercase;background:#c8f200;color:#111;border-radius:999px;padding:2px 8px">DEV 프리뷰</span>' +
    ...
    '<a href="/dev" style="' + btn('') + ';text-decoration:none">검수 콘솔</a>' +
  ...
  function refreshCount() {
    ...
    }).catch(function (e) { if (e && e.status === 403) { try { sessionStorage.removeItem('devPin'); } catch (x) {} location.href = '/dev'; } });
  }
