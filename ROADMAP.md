...
> (뒤 절반은 2026-09-02 에 폐기했다. 이 절 마지막 항목 참조)
...
- ⛔ **위 「빼기 = 삭제」 3건(PR #38)은 2026-09-02 에 폐기했다.** 되돌릴 수 없는 삭제가 「저장·배포」라는
  일상 동작에 딸려 붙어 사고가 났다. 기준선(`manual_base`)·삭제 예정 안내(`willRemove`)·배포 전 dryRun·
  `STALE_BASE` 409 를 모두 없애고 배포는 병합으로 되돌렸다. 「이 항목 빼기」는 편집 목록에서만 빼는 것이고,
  라이브 삭제는 `[🗑 라이브 항목 삭제]`(`/api/dev/live`) 한 곳에서만 일어난다 (PR #48)
