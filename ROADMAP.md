...
### 🌐 데이터 커버리지 확장
- [ ] **(기준 미정) AX 성숙도 축** — 기업이 AX 시장 진입 이후 어느 단계까지 왔는지 나타내는 축.
      분류(`category`)는 **기업 규모** 전용으로 확정했으므로(2026-08-12) 성숙도는 별개 축이 필요하다.
      진입 여부만은 `#AX신규진입` 태그로 이미 수집 중 → 그 다음 단계를 어떻게 나눌지가 미결.
      먼저 정해야 할 것: ① 단계 정의(예: 발표·PoC·상용 레퍼런스·조달 수주·플랫폼화)
      ② 판정 근거(무엇을 보고 단계를 올릴지) ③ 표현 방식(태그 vs 신규 필드).
