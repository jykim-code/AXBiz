...
### 첫 화면 로딩 표시 · 「빼기 = 삭제」 폐기 · 브랜치 정리  ✅  _(2026-09-02)_
> 어제 남은 것을 닫은 날. 되돌릴 수 없는 삭제를 일상 동작에서 떼어냈고,
> 「없음」과 「아직」이 화면에서 구분되게 했다.
- [x] **첫 화면 로딩이 「데이터 없음」으로 읽히던 문제** — 통계 초기값 `0`·`—` 가 3.6초 동안 그대로 보여
  「이번 기간에 동향이 하나도 없는 대시보드」로 읽혔다. 스켈레톤 카드와 `···`·「불러오는 중…」으로 교체 (PR #46)
- [x] **「이 항목 빼기」를 라이브 삭제에서 편집 목록 제거로 되돌린다** — 기준선(`manual_base`)·삭제 예정
  안내(`willRemove`)·dryRun·`STALE_BASE` 409 를 없애고 배포는 병합으로. 라이브 삭제는 `/api/dev/live` 한 곳뿐 (PR #48)
- [x] 문서 정정: 08-31 절의 「빼기 = 삭제」 3건에 폐기 표시, `CLAUDE.md` 에 **머지된 브랜치는 바로 정리한다**
  지침 추가(로컬·원격·워크트리 세 곳, 순서는 워크트리 → 로컬 → 원격) (PR #49)
- [x] TODO 에 09-01 사용성 점검 잔여 항목 등록 (PR #47), 09-01 머지분 ROADMAP 반영 (`docs/roadmap-0901`)
- [x] 브랜치 정리: 머지·대체된 브랜치를 로컬·원격·워크트리에서 제거. `fix/company-summary-style`(08-13)은
  별칭 사전(`e011cf9`)이 같은 이름 우선 매칭을 더 넓게 구현해 대체된 상태라 폐기(패치 백업 후 삭제)
- [x] 위클리 픽 배포보고(2026-08-24 스냅샷)를 저장소에 추가
