...
`weekly_edition`만 성격이 다르다. 나머지 파생 테이블은 `reports`에서 재생성할 수 있지만 이 테이블은
**발행 시점의 내용을 통째로 복사해 굳혀 둔 것**이라 원본에서 되살릴 수 없다. 단톡방에 뿌린 링크의
내용이 나중에 달라지면 공유물로 쓸 수 없기 때문이다(9절 참고). 백업 대상은 `reports`와 이 테이블 둘이다.
...
| **R2** `ax-biz-radar-img` | `env.IMG` | 위클리 픽 이미지. 키 = 내용 SHA-256 앞 32자 + 확장자 |

> R2 는 관리자가 올린 이미지만 담는다. 기사 사진을 자동 수집하지 않는다 — 핫링크는 우리 CSP
> (`img-src 'self'`)에 막히고, 원본이 사라지면 지난 회차가 소급해 깨진다. 이미지가 없으면 활자 판으로 나간다.

**stg 와 운영은 같은 D1·R2 를 쓴다.** `wrangler.toml` 에 환경별 오버라이드가 없어 바인딩이 하나뿐이다.
...
| 이미지 크롭 불가 | Workers 에 이미지 처리가 없어 잘리는 기준점만 저장한다. 교체 시 이전 R2 객체가 고아로 남고 정리 도구가 없다 | 10-1절 |
