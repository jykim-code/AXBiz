...
- 넘기기는 **코드가 직접 움직인다**(2026-09-01 개편). 트랙은 `overflow:hidden` + `touch-action:pan-y` 라
  native 가로 스크롤과 그 관성이 없고, 위치는 `scrollLeft` 를 코드가 정한다. 손가락은 끄는 동안
  판이 따라오고 손을 떼면 거리(판 폭 18%, 최대 90px) 또는 속도(0.35px/ms)로 **한 칸**을 정한다.
  트랙패드는 가로 성분이 있는 `wheel` 하나에 한 칸 넘기고, 이벤트가 140ms 넘게 끊길 때까지를
  같은 제스처로 묶는다. 세로는 브라우저에 남겨 본문 판(`.nw-scroll`)이 그대로 스크롤된다.
  - 이전 방식(가로 `scroll-snap`)을 버린 이유: 스냅은 「어디에 멈출지」만 정하고 관성을 막지 못해
    세게 밀면 스냅 지점 여러 개를 지나쳐 2~3장이 넘어갔다(데스크톱 트랙패드도 동일).
    `scroll-snap-stop:always` 는 iOS 관성에서 무시되는 경우가 있고, 손 뗀 뒤 되돌리는 보정은
    터치 이벤트가 없는 트랙패드에 걸리지 않았다.
